      ...
      <div className="sticky top-16 z-30 bg-[rgba(66,13,75,0.5)] backdrop-blur-xl border-b border-[rgba(123,51,126,0.2)]">
        <div className="mx-auto max-w-7xl px-4 py-4 flex items-center justify-between gap-4">
          <div className="flex flex-wrap gap-2">
            {CATEGORIES.map(c => (
              <button key={c} onClick={()=> setCategory(c)} className={`px-4 py-2 rounded-full border transition-all hover:shadow-[0_8px_30px_rgba(123,51,126,0.35)] ${category===c? 'bg-[#7B337E] border-transparent' : 'bg-[rgba(33,6,53,0.8)] border-[rgba(102,103,171,0.5)]'}`}>{c}</button>
            ))}
          </div>
